...
\commenta{there is actually nothing at all here about the platform! should include a paragraph about BURLAP somewhere, and should probably delete Platform from this section heading. and/or we need a section where we talk about our Platforms --- one of which is MTurk.}

Within the BURLAP infrastructure, we have built a flexible
experimental platform, in which it is straightforward to encode
experimental testbeds.  As an example, we describe one such testbed,
consisting of five such two-agent grid games, which were designed to
