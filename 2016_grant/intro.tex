...
\mydef{individually optimize} a \mydef{social utility function} representing
...
\commenta{3.~agents decide whether selfish or social behavior is apt;}
\commentm{seems like you aren't comfortable with adding this part in. it
seems important, though, given the goal including ``stance''
(are the agents acting jointly or not?).}
...
that social utility function.  Here, % in our computational model,
...
% social preferences exist extrinsically; rather, they are 
...
in the trace data.  While these two experiments
%  (on machine--machine,
% interactive and human--human batch data) 
...
that setting,
% these two market environments, 
rationality may well be an appropriate
...
humans in its environment selfishly follow
extrinsic utility functions is unlikely to be
...
behavior, this proposal assumes that humans are
