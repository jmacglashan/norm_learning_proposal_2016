...
we plan to build a ``go fetch'' application. In this application, a
user asks a mobile maipulator robot in the department to ``go fetch
object X from person Y'' (for example, ``borrow the HCI textbook from
Prof.\ Huang'' or ``pick up the package that Genie has for me in the
mail room'').
%on a mobile-manipulation platform that is scheduled for deployment in
%the Brown Computer Science department in Spring 2017.
